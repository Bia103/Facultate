...
class Culoare{
	...
	generateRandom(){
		this.r=randInt(0,256); //TO DO generati o culoare aleatoare folosind functia randInt definita mai sus
...
function ransom(sir)
{
	//document.body.style.background=grey;//TO DO setati culoarea de background a body-ului la gri
	...
	for (i=0;i<sir.length;i++) //TO DO parcurgere sir
		{
			...
			var patt1=/\s/g; 					// spatiu

			if(!sir[i].match(patt1))			//daca nu e spatiu
			{
				...
				nr=randInt(0,2);
				lit.style.fontStyle=fonturi[ransInt(0,fonturi.length)]; // TO DO - setare random  daca sa fie font italic sau nu (ca si la bold)
				fonturi=["Times New Roman","Comic Sans MS","Impact","Arial Black","Courier New","Lucida Console","Trebuchet MS"];
				lit.style.fontFamily=fonturi[ransInt(0,fonturi.length)]; //TO DO element aleator din fonturi
				lit.onclick= function(){
					console.log(this.style.color+" "+this.style.background);// TO DO afisati in consola culoarea elementului (atat de text cat si de background)
				}
			}
			else
			{
